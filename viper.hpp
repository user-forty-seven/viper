...
struct ConfigError
{
    ...
    { 
        return type != ConfigErrorType::NONE; 
    }
};
...
inline FlagValidator Int()
{
    return [](const std::string& v) -> std::optional<std::string>
    {
        if (v.empty())
        {
            return "must be an integer, got empty string";
        }
        std::size_t pos = 0;
        try
        {
...
inline FlagValidator AllOf(std::vector<FlagValidator> validators)
{
    return [validators = std::move(validators)]
...
std::function<int(const std::vector<std::string>&)> wrapRun(F&& f)
{
    ...
    else
    {
        static_assert(always_false_v<F>, 
            "run callback must be callable"
            "(either as () or (const std::vector<std::string>&)" 
            ", and must return either void or int");
    }
}
//
...
struct Command
{
    ...
    Command* addCmd(
        ...
        std::string cmdDesc,
        F&& cmdRun, 
        const ArgValidator& cmdArgs = AnyArgs()
        )
    {
        if (childDuplicate(cmdName))
        {
            g_ConfigErrors.push_back(
                ConfigError{
                    ConfigErrorType::DUPLICATE_CHILD, 
    ...
    template<
        ...
        >
    >
    Flag* addFlagPersistentRequired(
        const std::string& flagName, 
        const char shortName, 
        T& var, 
        const std::string& cmdDesc = "", 
    ...
    Flag* flagLookup(const std::string& flagName) const
    {
        for (auto& f : flags)
        {
            bool matchesLongName  = f->longName == flagName;
            bool matchesShortName = 
                flagName.length() == 1 && 
                flagName[0] == f->shortName;

            if (matchesLongName || matchesShortName)
            {
                return f.get();
            }
        }
        for (auto c = this->parent; c; c = c->parent)
        {
            for (auto& pf : c->flags)
            {
                bool matchesLongName  = pf->longName == flagName;
                bool matchesShortName = 
                    flagName.length() == 1 && 
                    flagName[0] == pf->shortName;

                if (pf->persistent && (matchesLongName || matchesShortName)) 
                {
                    return pf.get();
                }
            }
...
{
    return [validators = std::move(validators)]
        (std::size_t count) -> std::optional<std::string>
    {
        for (auto& validator : validators)
        {
...
inline ParseError parse(
    Command*& current, 
    const std::vector<Token>& tokenized, 
...
inline int execute(
    Command* root, 
    int argc, char** argv, 
    const ErrorHandler& errorHandler = defaultErrorHandler
    )
{
...
inline void addDefaults(Command* cmd)
{
    ...
    {
        cmd->addFlag(
            "help", 'h', cmd->helpRequested, "show this help message"
        );
    }
